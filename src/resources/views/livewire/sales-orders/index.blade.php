...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500">{{ $totalOrders }} total orders</p>
        </div>
        <a href="{{ route('sales-orders.create') }}" wire:navigate class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition shadow-sm">
        ...
        <div class="p-4 border-b border-gray-100 space-y-3">
            <div class="flex flex-col sm:flex-row gap-3">
                <div class="flex-1 relative">
                    ...
                    <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search by order # or customer..." class="w-full pl-10 pr-4 py-2.5 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none">
                </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-left">
                    <tr>
                        ...
                        <th class="px-6 py-3 font-medium text-gray-500 hidden lg:table-cell">Delivery Date</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4">
                            <span class="px-2.5 py-0.5 rounded-full text-xs font-medium {{ $this->getStatusColor($order->status) }}">{{ ucfirst($order->status) }}</span>
                        </td>
                        <td class="px-6 py-4 text-gray-500 hidden lg:table-cell">{{ $order->delivery_date?->format('M d, Y') ?? '-' }}</td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1" x-data>
                                <a href="{{ route('sales-orders.show', $order) }}" wire:navigate class="p-2 text-gray-400 hover:text-blue-600 rounded-lg hover:bg-blue-50 transition" title="View">
                                    <i class="fas fa-eye text-sm"></i>
                                </a>
                                <a href="{{ route('sales-orders.edit', $order) }}" wire:navigate class="p-2 text-gray-400 hover:text-amber-600 rounded-lg hover:bg-amber-50 transition" title="Edit">
                                    <i class="fas fa-pen text-sm"></i>
                                </a>
                                <button wire:click="confirmDelete({{ $order->id }})" class="p-2 text-gray-400 hover:text-red-600 rounded-lg hover:bg-red-50 transition" title="Delete">
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-medium text-gray-900">No sales orders found</p>
                                    <p class="text-xs text-gray-500">Create your first order to get started</p>
                                </div>
                            </div>
                        </td>
                    </tr>
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div class="relative bg-white rounded-xl shadow-xl max-w-md w-full p-6 space-y-4">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900">Delete Sales Order</h3>
                </div>
                <p class="text-sm text-gray-500">Are you sure you want to delete this sales order? This action cannot be undone.</p>
